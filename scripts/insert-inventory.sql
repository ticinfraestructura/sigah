-- Insertar inventario de kits y lotes de productos
...
-- Movimientos de inventario de kits
INSERT INTO kit_inventory_movements (id, "kitInventoryId", type, quantity, "lotNumber", reason, reference, "userId", "createdAt") VALUES
  ('990e8400-e29b-41d4-a716-446655440001', '880e8400-e29b-41d4-a716-446655440001', 'ENTRY', 10, 'KL001', 'Inventario inicial', 'INIT-001', (SELECT id FROM users WHERE email='admin@test.com'), NOW()),
  ('990e8400-e29b-41d4-a716-446655440002', '880e8400-e29b-41d4-a716-446655440002', 'ENTRY', 5, 'KL002', 'Inventario inicial', 'INIT-002', (SELECT id FROM users WHERE email='admin@test.com'), NOW())
ON CONFLICT DO NOTHING;

-- Solicitudes de prueba
INSERT INTO requests (id, code, "beneficiaryId", status, priority, "createdById", "createdAt", "updatedAt") VALUES
  ('aa0e8400-e29b-41d4-a716-446655440001', 'SOL-001', (SELECT id FROM beneficiaries LIMIT 1), 'PENDING', 'HIGH', (SELECT id FROM users WHERE email='admin@test.com'), NOW(), NOW()),
  ('aa0e8400-e29b-41d4-a716-446655440002', 'SOL-002', (SELECT id FROM beneficiaries OFFSET 1 LIMIT 1), 'APPROVED', 'MEDIUM', (SELECT id FROM users WHERE email='admin@test.com'), NOW(), NOW())
ON CONFLICT DO NOTHING;

SELECT 'Inventario creado:' as info;
SELECT 'Product Lots: ' || count(*) FROM product_lots;
SELECT 'Kit Inventory: ' || count(*) FROM kit_inventory;
SELECT 'Kit Movements: ' || count(*) FROM kit_inventory_movements;
